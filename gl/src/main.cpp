...
//    const double dt = 0.01; // 采样间隔
//    const double f0 = 10.0; // 信号频率
//    const double A = 1.0; // 信号振幅
//
//    // 分配输入和输出数组
//    fftw_complex* in = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * N);
//    fftw_complex* out = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * N);
//
//    // 创建 FFTW 进行离散傅里叶变换的计划
//    fftw_plan plan = fftw_plan_dft_1d(N, in, out, FFTW_FORWARD, FFTW_ESTIMATE);
//
//    // 生成信号
//    for (int i = 0; i < N; ++i) {
//        double t = i * dt;
//        in[i][0] = A * std::sin(2.0 * 3.14 * f0 * t); // 实部
//        in[i][1] = 0.0; // 虚部
//    }
//
//    // 执行傅里叶变换
...
//    // 计算频谱
//    for (int i = 0; i < N; ++i) {
//        double freq = i / (N * dt); // 频率
//        double magnitude = sqrt(out[i][0] * out[i][0] + out[i][1] * out[i][1]); // 幅度
